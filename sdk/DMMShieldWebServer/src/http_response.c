...
{
	int BUFSIZE = 1024;
	unsigned char buf[BUFSIZE], szMsg[200];
	int len, valLen;
	double dMeasuredVal;
    ...
    if (is_cmd_scale(req, &idxScale))
	{
		// set scale
    	DMM_SetScale(idxScale);// send the selected configuration to the DMM
        if(bErrCode == ERRVAL_SUCCESS)
        {
            curScale = idxScale;
			szMsg[0] = '0' + (unsigned char)(curScale / 10);
			szMsg[1] = '0' + (unsigned char)(curScale % 10);
			szMsg[2] = 0; // terminating 0;
        }
        else
        {
        ...
        valLen = strlen(szMsg);
		len = generate_http_header((char *)buf, "js", valLen);
        p = (char *)buf + len;
        strcpy(p, szMsg);
        len += valLen;
        ...
        if (is_cmd_val(req))
    	{
			if(curScale != -1)
			{
				...
				if(bErrCode == ERRVAL_SUCCESS)
				{
					...
					szMsg[0] = '0' + (unsigned char)(curScale / 10);
					szMsg[1] = '0' + (unsigned char)(curScale % 10);
					DMM_FormatValue(dMeasuredVal, szMsg + 2, 1);
				}
				else
				{
					bErrCode = ERRORS_GetPrefixedMessageString(bErrCode, "", szMsg);
