...
int
is_cmd_scale(char *buf, int *pscale)
{
	int result = 0;
	*pscale = 0;
	...
	/* then check for cmd/sclxhr */
	result = !strncmp(buf, "cmd", 3) && !strncmp(buf + 4, "sclxhr", 6);
	if(result)
	{
		*pscale = atoi(buf+10);
	}
...
is_cmd_val(char *buf)
{
	...
	/* then check for cmd/sclxhr */
	result = !strncmp(buf, "cmd", 3) && !strncmp(buf + 4, "valxhr", 6);
